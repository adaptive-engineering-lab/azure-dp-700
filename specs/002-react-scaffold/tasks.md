...
- [X] T012 [P] Create `frontend/src/lib/storage/namespace.ts` exporting `KEY = 'dp700game.v1.state'`, `PROBE_KEY = 'dp700game.probe'`, and `CURRENT_VERSION = 1`.
...
- [X] T019 [P] Create `frontend/tests/unit/namespace.test.ts` (FR-011): asserts the namespace constants, the `dp700game.*` prefix, and that no other key the app writes escapes the prefix (regex pass over the source).
...
**Goal**: All four state slices (`preferences`, `profile`, `progress`, `sessions`) persist to localStorage under `dp700game.v1.state`; theme survives a reload with no FOUC; missing-localStorage browsers fall through to in-memory state with a one-time dismissible notice.
...
- [X] T056 [US2] Create `frontend/src/lib/store/index.ts` — composes all four slices via Zustand's `combine`. Wraps with the `persist` middleware pointed at the `storage` adapter from T015, key `KEY` (=`dp700game.v1.state`), `version: CURRENT_VERSION`, and `migrate: migrate` from T014. Exposes `useStore` hook.
